...
namespace TestCols
{
    class Program
    {
        static void Main()
        {
            string connStr = "Data Source=(LocalDB)\\MSSQLLocalDB;AttachDbFilename=C:\\Users\\ivanc\\source\\repos\\wapp\\App_Data\\CyberShieldAcademy.mdf;Integrated Security=True;MultipleActiveResultSets=True";
            
            // Get all students
            var students = new[] { "USRFEI001", "USRGMP204", "USRJOY656", "USRJWY112", "USRKKC600", "USRRBX787" };
            
            foreach (var studentId in students)
            {
                Console.WriteLine($"\n=== Testing student: {studentId} ===");
                TestStudent(connStr, studentId);
            }
        }
        
        static void TestStudent(string connStr, string studentId)
        {
            using (var con = new SqlConnection(connStr))
            using (var cmd = new SqlCommand($@"
                DECLARE @StudentID NVARCHAR(10) = '{studentId}';
                SELECT
                    q.QuizID,
                    q.Title,
                    q.PassMark,
