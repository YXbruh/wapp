    ...
    public partial class Site : MasterPage
    {
        ...
        private string CurrentProfilePicture
        {
            get
            {
                ...
                string picture = "";
                DataRow row = ProfileService.Get(Session["UserID"].ToString());
                if (row != null && row["ProfilePicture"] != DBNull.Value)
                    picture = Convert.ToString(row["ProfilePicture"]);

                // Cache "" too, so a user without a picture is only looked up once.
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            pnlLecturerNav.Visible = isLoggedIn && role == "Lecturer";
        ...
        protected override void OnPreRender(EventArgs e)
        {
            ...
            string picture = CurrentProfilePicture;

            if (!string.IsNullOrEmpty(picture))
            {
                string src = HttpUtility.HtmlAttributeEncode(ResolveUrl(picture));
                lbProfile.InnerHtml = $"<img src=\"{src}\" alt=\"\" class=\"nav-avatar-img\" />";
            }
            else
            {
                lbProfile.InnerHtml = ProfileService.MakeInitials(name);
            }

            lbProfile.HRef = role == "Admin"
                ? ResolveUrl("~/Admin/Profile.aspx")
                : role == "Lecturer"
                    ? ResolveUrl("~/Lecturer/Profile.aspx")
        ...
        private void HighlightActiveLink()
        {
            ...
            string script = $"document.querySelectorAll('.nav-link[href]').forEach(function(a){{" +
                            ...
                            $"  }}" +
                            $"}});";
            Page.ClientScript.RegisterStartupScript(GetType(), "activeNav", script, true);
        }
    }
}