...
namespace CSA.Services
{
    ...
    /// the instructorId and filters on it, so one lecturer can't view arbitrary
    ...
    public static class StudentDetailService
    {
        ...
        /// </summary>
        public static bool Owns(string instructorId, string studentId)
        {
        ...
        public static DataRow GetProfile(string studentId, string instructorId)
        {
                ...
                SELECT  u.FullName,
                        ...
                        (SELECT COUNT(*)
                         ...
                         WHERE e2.StudentID = u.UserID
                           AND c2.InstructorID = @InstructorID) AS EnrolledCount
                FROM    Users u
                ...
                SELECT
                    ...
                    ISNULL((
                        ...
                        FROM (
                            ...
                            WHERE  qa.StudentID = @StudentID
                              AND  q.CreatedByID = @InstructorID
                            GROUP BY qa.QuizID
                        ) best
                    ), 0) AS QuizAvg,
                    ...
                    (SELECT COUNT(DISTINCT ls.LabID)
                     ...
                     WHERE  ls.StudentID = @StudentID
                       ...
                       AND  vl.CreatedByID = @InstructorID) AS LabsDone,
        ...
        /// </summary>
        public static DataTable GetQuizAttempts(string studentId, string instructorId)
        {
        ...
        /// </summary>
        public static string GetSharedCourseId(string instructorId, string studentId)
        {
            object result = DBHelper.ExecuteScalar(@"
                SELECT TOP 1 c.CourseID
                FROM   Enrollments e
                INNER JOIN Courses c ON e.CourseID = c.CourseID
                WHERE  e.StudentID = @StudentID AND c.InstructorID = @InstructorID
        ...
        /// </summary>
        public static void SendFeedback(string instructorId, string studentId, string message)
        {
            string courseId = GetSharedCourseId(instructorId, studentId);

            DBHelper.ExecuteNonQuery(@"
                INSERT INTO Feedback (StudentID, LecturerID, CourseID, StarRating, Comment, SubmittedAt, RepText, RepAt)
                VALUES (@StudentID, @InstructorID, @CourseID, NULL, NULL, GETDATE(), @RepText, GETDATE());",
                new SqlParameter("@StudentID", studentId),
                new SqlParameter("@InstructorID", instructorId),
        ...
        ///          CompletedDisplay.
        /// </summary>
        public static DataTable GetLabStatus(string studentId, string instructorId)
        {
            return DBHelper.ExecuteQuery(@"
                ...
                WHERE   vl.IsPublished = 1
                  AND   vl.CreatedByID = @InstructorID
