...
namespace CSA.Student
{
    public partial class Student_Challenges : Page
    {
        ...
        private const string TimerStartPrefix = "StudentQuizTimerStart_";
        private const string TimerDurationPrefix = "StudentQuizTimerDuration_";
        private const string AttemptSubmittedPrefix = "StudentQuizAttemptSubmitted_";
        ...
        {
            get { return TimerStartPrefix + UserId + "_" + QuizId; }
        }

        private string TimerDurationKey
        {
            get { return TimerDurationPrefix + UserId + "_" + QuizId; }
        }

        private string AttemptSubmittedKey
        {
            get { return AttemptSubmittedPrefix + UserId + "_" + QuizId; }
        }
        ...
        private bool CurrentAttemptSubmitted
        {
            get
            {
                object value = Session[AttemptSubmittedKey];
                return value != null && Convert.ToBoolean(value);
            }
            set { Session[AttemptSubmittedKey] = value; }
        }
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            if (IsPostBack && !string.IsNullOrWhiteSpace(QuizId))
            {
                BindQuestions();
            }
        }
        ...
        private void LoadChallenges()
        {
            ...
            litPassed.Text = dt.AsEnumerable()
                .Count(row => Convert.ToBoolean(row["HasPassed"]))
                .ToString();
            litAttempts.Text = dt.AsEnumerable()
                .Sum(row => Convert.ToInt32(row["AttemptCount"]))
                .ToString();
            ...
            RepeaterCommandEventArgs e)
        {
            ...
            DataTable quizTable = Query(@"
                SELECT ISNULL(MaxAttempts, 3) AS MaxAttempts
                FROM Quizzes
                WHERE QuizID = @QuizID
                  AND IsPublished = 1",
            ...
            int maxAttempts = GetMaxAttempts(quizTable.Rows[0]);

            // Reopening from the challenge list starts the next attempt.
            if (attempts < maxAttempts && CurrentAttemptSubmitted)
            {
        ...
        private void LoadQuiz()
        {
            ...
            int attempts = GetAttemptCount();
            int maxAttempts = GetMaxAttempts(quiz);

            litQuizTitle.Text = Server.HtmlEncode(Convert.ToString(quiz["Title"]));
            litQuizDescription.Text =
                Server.HtmlEncode(Convert.ToString(quiz["Description"])) +
                BuildAttachmentsHtml(GetQuizAttachments());
            litCourseName.Text = Server.HtmlEncode(Convert.ToString(quiz["CourseName"]));
            litPassMark.Text = Convert.ToDecimal(quiz["PassMark"]).ToString("0.##");
            litAttemptUsage.Text = attempts + " / " + maxAttempts + " attempts";

            string notice = GetUnavailableMessage(quiz, attempts, maxAttempts);
            pnlNotice.Visible = notice != "";
            litNotice.Text = Server.HtmlEncode(notice);

            hlQuizFeedback.Visible = attempts > 0;
            hlQuizFeedback.NavigateUrl =
                "Feedback.aspx?type=quiz&id=" + Server.UrlEncode(QuizId);
            ...
            if (attempts >= maxAttempts)
            {
                ...
                litNotice.Text =
                    ...
                    "The questions, options, correct answers and explanations are shown below.";
            }
            else if (CurrentAttemptSubmitted)
            {
                ...
                litNotice.Text =
                    ...
                    "Return to the Challenges page to begin another attempt.";
            }
            else
            {
                btnSubmit.Visible = true;
                btnSubmit.Enabled = notice == "";
                ConfigureTimer(quiz, attempts, maxAttempts);
            }
        ...
        private static int GetMaxAttempts(DataRow quiz)
        {
            if (quiz["MaxAttempts"] == DBNull.Value)
            {
                return 3;
            }

            int maxAttempts = Convert.ToInt32(quiz["MaxAttempts"]);
            ...
            int maxAttempts)
        {
            ...
            if (!btnSubmit.Enabled ||
                ...
                quiz["DurationMinutes"] == DBNull.Value)
            {
                return;
            }

            int durationMinutes = Convert.ToInt32(quiz["DurationMinutes"]);
            ...
            if (Session[TimerStartKey] == null)
            {
                startedAt = DateTime.UtcNow;
                Session[TimerStartKey] = startedAt;
                Session[TimerDurationKey] = durationMinutes;
            }
            else
            {
                startedAt = Convert.ToDateTime(Session[TimerStartKey]);
            }

            int storedDuration = Session[TimerDurationKey] == null
                ? durationMinutes
                : Convert.ToInt32(Session[TimerDurationKey]);

            DateTime expiresAt = startedAt.AddMinutes(storedDuration);
            int remainingSeconds = Convert.ToInt32(
                Math.Ceiling((expiresAt - DateTime.UtcNow).TotalSeconds));
            ...
            hfRemainingSeconds.Value = remainingSeconds.ToString();
        }

        private bool HasTimeExpired()
        {
            if (Session[TimerStartKey] == null ||
                Session[TimerDurationKey] == null)
            {
                return false;
            }

            DateTime startedAt = Convert.ToDateTime(Session[TimerStartKey]);
            int durationMinutes = Convert.ToInt32(Session[TimerDurationKey]);

            return DateTime.UtcNow >= startedAt.AddMinutes(durationMinutes);
        }
        ...
        private void ClearTimer()
        {
            ...
            {
                Session.Remove(TimerStartKey);
                Session.Remove(TimerDurationKey);
            }
            ...
            RepeaterItemEventArgs e)
        {
            ...
            Panel mcqPanel = (Panel)e.Item.FindControl("pnlMCQ");
            RadioButtonList trueFalse =
                (RadioButtonList)e.Item.FindControl("rblTrueFalse");
            TextBox structure = (TextBox)e.Item.FindControl("tbStructure");
            Literal attachments =
                (Literal)e.Item.FindControl("litQuestionAttachments");

            attachments.Text = BuildAttachmentsHtml(
                GetQuestionAttachments(
                    Convert.ToString(row["QuestionID"])));

            mcqPanel.Visible = type == "MCQ";
            trueFalse.Visible = type == "TrueFalse";
        ...
        private DataTable GetQuizAttachments()
        {
            return Query(@"
                SELECT
                    AttachmentType,
                ...
                FROM Attachments
                WHERE QuizID = @QuizID
                ORDER BY UploadedAt, AttachmentID",
                new SqlParameter("@QuizID", QuizId));
        }

        private DataTable GetQuestionAttachments(
            string questionId)
        {
            return Query(@"
                SELECT
        ...
        private string BuildAttachmentsHtml(
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    btnSubmit.Enabled = false;
                    ShowResult(false, "This attempt has already been submitted.");
                    return;
                }
            ...
            try
            {
                DataTable quizTable = Query(@"
                    SELECT
                        ...
                        EndDate,
                        DurationMinutes,
                        ISNULL(MaxAttempts, 3) AS MaxAttempts,
                        ISNULL(
                            TotalMarks,
                            (
                ...
                string unavailable =
                    GetUnavailableMessage(quiz, attempts, maxAttempts);

                bool timeExpired =
                    HasTimeExpired() || hfRemainingSeconds.Value == "0";

                if (unavailable != "" && !timeExpired)
                {
                    CurrentAttemptSubmitted = false;
                    btnSubmit.Enabled = true;
                ...
                bool passed = score >= Convert.ToDecimal(quiz["PassMark"]);
                bool awardXp = passed && !HasPassed();
                ...
                {
                    message +=
                        " The quiz was submitted because the time limit ended.";
                }
                ...
                else
                {
                    ...
                    litNotice.Text =
                        "Attempt submitted. Return to the Challenges page " +
                        "and click Reattempt when you are ready.";
                }
        ...
        private void SetAnswerControlsEnabled(bool enabled)
        {
            ...
            {
                CheckBox cbA = item.FindControl("cbA") as CheckBox;
                CheckBox cbB = item.FindControl("cbB") as CheckBox;
                CheckBox cbC = item.FindControl("cbC") as CheckBox;
                CheckBox cbD = item.FindControl("cbD") as CheckBox;
                RadioButtonList trueFalse =
                    item.FindControl("rblTrueFalse") as RadioButtonList;
                TextBox structure = item.FindControl("tbStructure") as TextBox;

                if (cbA != null) cbA.Enabled = enabled;
                if (cbB != null) cbB.Enabled = enabled;
                if (cbC != null) cbC.Enabled = enabled;
                if (cbD != null) cbD.Enabled = enabled;
                if (trueFalse != null) trueFalse.Enabled = enabled;
                if (structure != null) structure.Enabled = enabled;
            }
        }
        ...
        private void ShowCorrectAnswersOnly()
        {
            ...
            foreach (RepeaterItem item in rptQuestions.Items)
            {
                ...
                DataRow question = questions.AsEnumerable()
                    .First(row =>
                        Convert.ToString(row["QuestionID"]) == questionId);

                Panel studentAnswerPanel =
                    item.FindControl("pnlStudentAnswer") as Panel;
                Panel answerReviewPanel =
                    item.FindControl("pnlAnswerReview") as Panel;
                Panel explanationPanel =
                    item.FindControl("pnlExplanation") as Panel;
                Label answerResult =
                    item.FindControl("lblAnswerResult") as Label;
                Literal correctAnswer =
                    item.FindControl("litCorrectAnswer") as Literal;
                Literal explanation =
                    item.FindControl("litExplanation") as Literal;

                // Keep the question and its options visible, but locked.
                if (studentAnswerPanel != null)
                {
                    studentAnswerPanel.Visible = false;
                }
                ...
                {
                    correctAnswer.Text = Server.HtmlEncode(
                        DisplayAnswer(
                            question,
                ...
                if (explanation != null)
                {
                    explanation.Text = Server.HtmlEncode(explanationText);
                }

                if (answerReviewPanel != null)
                {
                    answerReviewPanel.Visible = true;
                }
            }
        }
        ...
        private Dictionary<string, string> ReadAnswers()
        {
            ...
            foreach (RepeaterItem item in rptQuestions.Items)
            {
                ...
                {
                    answers[questionId] =
                        ((RadioButtonList)item.FindControl("rblTrueFalse"))
                            .SelectedValue;
                }
                else
                {
                    answers[questionId] =
                        ((TextBox)item.FindControl("tbStructure"))
                            .Text.Trim();
                }
            }
        ...
        private static bool IsCorrect(DataRow question, string submitted)
        {
            string type = Convert.ToString(question["QuestionType"]);
            string expected = Convert.ToString(question["CorrectAnswer"]);
            ...
            Dictionary<string, string> answers)
        {
            foreach (RepeaterItem item in rptQuestions.Items)
            {
                ...
                bool correct = IsCorrect(question, answer);
                Label result =
                    (Label)item.FindControl("lblAnswerResult");
                Panel studentAnswerPanel =
                    item.FindControl("pnlStudentAnswer") as Panel;

                if (studentAnswerPanel != null)
                {
                    studentAnswerPanel.Visible = true;
                }

                result.Visible = true;
                result.Text = correct ? "Correct" : "Incorrect";
                result.CssClass = correct
                    ? "badge badge-green"
                    : "badge badge-red";

                string studentAnswer = string.IsNullOrWhiteSpace(answer)
                    ? "No answer"
                    : DisplayAnswer(question, answer);

                string correctAnswer = DisplayAnswer(
                    question,
                    Convert.ToString(question["CorrectAnswer"]));

                ((Literal)item.FindControl("litStudentAnswer")).Text =
        ...
        private static string DisplayAnswer(DataRow question, string answer)
        {
            ...
            foreach (string letter in Normalise(answer).Split(','))
            {
                ...
                string option =
                    Convert.ToString(question["Option" + letter]);
                result.Add(letter + ". " + option);
            }
                ...
                using (SqlTransaction transaction = con.BeginTransaction())
                {
                    try
                    {
                        Execute(con, transaction, @"
                            ...
                            new SqlParameter("@TotalMarks", total),
                            new SqlParameter("@ObtainedMarks", obtained),
                            new SqlParameter("@IsPassed", passed));
                        ...
                        if (awardXp)
                        {
                            Execute(con, transaction, @"
                                ...
                                new SqlParameter("@Points", obtained),
                                new SqlParameter("@StudentID", UserId));
                        }
        ...
        {
            // Do not clear an unfinished attempt's timer here.
            QuizId = "";
